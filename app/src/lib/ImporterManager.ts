...
export class ImporterManager {
  constructor(private readonly workflowClient: WorkflowClient, private readonly mongoClient: MongoClient) {}
  ...
  public async getMappingRecommendations(importerId: string): Promise<DataMappingRecommendation[]> {
    const handle = this.workflowClient.getHandle(importerId);
    ...
    const recommendations = await handle.query<DataMappingRecommendation[]>("importer:data-mapping-recommendations");
  ...
  public async updateColumnValidation(importerId: string, columnConfigKey: string, columnValidation: ColumnValidation) {
    const handle = this.workflowClient.getHandle(importerId);
    const workflowState = await handle.query<ImporterStatus>("importer:status");
    if (workflowState.state === "closed") {
      throw new Error("Importer is closed");
  ...
  public async resetImporter(importerId: string): Promise<void> {
    ...
    const waitForRunning = async () => {
      ...
      const isNewWorkflowRunning =
        status.status.code === temporal.api.enums.v1.WorkflowExecutionStatus.WORKFLOW_EXECUTION_STATUS_RUNNING;
      const hasStartedNewWorkflowRun = previousRunStatus.runId !== status.runId && isNewWorkflowRunning;
