...
const ValidationTable = (props: Props) => {
  ...
  }, [importerId, onReloadConfig, props.importerDto.config.columnConfig, props.importerDto.status.dataMapping, t]);
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow key={headerGroup.id} className="flex w-full">
                  {headerGroup.headers.map((header) => {
                    const numberOfMessages = props.importerDto.status.meta?.messageCount[header.column.id] ?? 0;
                    return (
                      <TableHead
                        className="border-r"
                        key={header.id}
