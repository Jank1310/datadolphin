...
import type { RecordUpdateResult } from "@/app/api/importer/[slug]/records/RecordUpdateResult";
import {
	type FetchRecordsFilter,
	useFetchRecords,
} from "@/components/hooks/useFetchRecords";
...
const Validation = ({
	...
	const handleRecordUpdate = React.useCallback(
		(
			...
			if (result.changedColumns.length > 0) {
				handleLoadPage(rowPage, currentFilter, true);
			}
	...
	const handleUploadDifferentFile = React.useCallback(async () => {
		...
		try {
			const res = await frontendFetch(
				`/api/importer/${initialImporterDto.importerId}/reset`,
				{
					method: "POST",
				},
			);
		...
		<div>
					...
					>
						{t(
							currentFilter?.errors
								? "validation.btnShowAll"
								: "validation.btnShowOnlyErrors",
						)}
					</Button>
					<Button
						disabled={isResetting}
			...
			<div className="px-4">
				<ValidationTable
					...
					onLoadPage={(page, force) =>
						handleLoadPage(page, currentFilter, force)
					}
