"use client";
import { DataMappingRecommendation, ImporterDto } from "@/app/api/importer/[slug]/ImporterDto";
...
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/basicTable";
...
const SelectMappings = ({ initialImporterDto, initialDataMappingsRecommendations }: Props) => {
    ...
    const { recommendations: dataMappingRecommendations } = useGetMappingRecommendations(
        initialImporterDto.importerId,
        enablePolling ? 500 : undefined,
        { recommendations: initialDataMappingsRecommendations }
    );

    const isWaitingForMappingRecommendations = dataMappingRecommendations === null;
    ...
    const handleChangeMapping = (sourceColumn: string, newTargetColumn: string | null) => {
    ...
    const handleUploadDifferentFile = React.useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast({
                    title: t("mappings.toast.errorResetting"),
                    variant: "destructive",
                });
            }
        } catch (err) {
        ...
        return (
            <div className="w-full h-full flex justify-center items-center">
                <div className="flex flex-col items-center">
                    <span className="text-slate-500">{t("mappings.waitingForMappings")}</span>
    ...
    if (isMappingData) {
        return (
            <div className="w-full h-full flex justify-center items-center">
                <div className="flex flex-col items-center">
                    <span className="text-slate-500">{t("mappings.applyingMappings")}</span>
                    <LoadingSpinner className="text-slate-500 mt-2" />
                </div>
            </div>
        );
            ...
            <div className="h-14 flex justify-between items-center px-4">
                ...
                <div className="flex gap-2">
                    <Button disabled={isResetting} variant="secondary" onClick={handleUploadDifferentFile}>
                        {t("mappings.btnUploadDifferentFile")}
                    </Button>
                    <Button disabled={isSavingMapping} onClick={handleSaveMapping}>
                        {t("mappings.btnConfirmMappings")} <ChevronRightCircleIcon className="ml-2" />
                    </Button>
                </div>
            </div>
            <div className="px-4">
                <div className="rounded-lg border bg-card text-card-foreground shadow-sm mb-4 h-[calc(100vh_-_4.5rem)] overflow-auto">
                    <Table className="">
                        <TableHeader>
                            <TableRow>
                                <TableHead className="w-1/2">{t("mappings.columnsInYourFile")}</TableHead>
                                <TableHead className="w-1/2">{t("mappings.targetColumns")}</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
