...
export async function importer(params: ImporterWorkflowParams) {
      ...
      try {
        ...
        for (const patch of updateParams.patches) {
          ...
          if (columnValidations?.length) {
            ...
            const validationResultsGroupedByColumn = mapValues(keyBy(validationResults, "column"), "messages");
  ...
  try {
    ...
    const allMappedColumnsWithValidators = columnConfig.filter(
      (column) =>
        column.validations?.length && (configuredMappings ?? []).find((mapping) => mapping.targetColumn === column.key)
    );
...
function mergeEnumValidations(configs: ColumnConfig[]): ColumnConfig[] {
  return configs.map((config) => {
    if (config.validations) {
      const enumValidations = config.validations.filter((val) => val.type === "enum") as EnumerationColumnValidation[];
