...
export function makeActivities(
    ...
    return {
        ...
        processSourceFile: async (params: {
            ...
            switch (params.format) {
                        ...
                        (resolve, reject) => {
                            csv.parse(
                                fileData,
                                {
                                    columns: true,
                                    delimiter:
                                        params.formatOptions.delimiter ?? ",",
                ...
                case "xlsx":
                    ...
                    json = XLSX.utils.sheet_to_json<Record<string, unknown>>(
                        worksheet,
                        {
        ...
        applyMappings: async (params: {
            ...
            const sourceJsonData = await database.getSourceData(
                params.importerId
            );
        ...
        getMappingRecommendations: async (params: {
            ...
            const firstRecord = await database.getFirstSourceRow(
                params.importerId
            );
            if (!firstRecord) {
                return [];
        ...
        processDataValidationForRecord: async (params: {
            ...
            const row = await database.getDataRecord(
                params.importerId,
                params.rowId
            );
            if (!row) {
        ...
        generateStatsPerColumn: async (params: {
            ...
            const totalCount = await database.getTotalRecordsCount(
                params.importerId
            );
