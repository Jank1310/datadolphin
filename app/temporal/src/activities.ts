...
import type { DataSet, DataSetPatch, DataSetRow, SourceDataSet, SourceDataSetRow } from "./domain/DataSet";
import { Database } from "./infrastructure/Database";
import { FileStore } from "./infrastructure/FileStore";
import type { ImporterStatus, Mapping, Meta } from "./workflows/importer.workflow";
...
export function makeActivities(fileStore: FileStore, database: Database, dataAnalyzer: DataAnalyzer) {
    return {
        ...
        processSourceFile: async (params: {
            ...
            const fileData = await fileStore.getFile(params.importerId, params.fileReference);
            let json: Record<string, unknown>[];
            switch (params.format) {
                case "csv":
                    json = await new Promise<Record<string, unknown>[]>((resolve, reject) => {
                        csv.parse(
                            fileData,
                            {
                                columns: true,
                ...
                case "xlsx":
                    ...
                    break;
                default:
                    throw ApplicationFailure.nonRetryable(`Unsupported format ${params.format}`);
            }
            ...
            console.time("insert");
            const insertedCount = await database.insertSourceData(params.importerId, jsonWithRowIds);
        ...
        applyMappings: async (params: { importerId: string; dataMapping: Mapping[] }): Promise<void> => {
            ...
            const mappingsWithTargetColumn = params.dataMapping.filter((mapping) => mapping.targetColumn);
        ...
        getMappingRecommendations: async (params: {
            ...
            const allEmptyColumns = Object.keys(firstRecord).filter((key) => key.startsWith("__EMPTY") || key === "");
            const sourceColumns = pullAll(Object.keys(firstRecord as SourceDataSetRow), [
                "__sourceRowId",
                "_id",
                ...allEmptyColumns,
            ]);
            return dataAnalyzer.generateMappingRecommendations(sourceColumns, params.columnConfig);
        },
        processDataValidations: async (params: {
            ...
            const jsonData: DataSet = await database.getData(params.importerId, params.skip, params.limit);
            ...
            await database.updateDataWithValidationMessages(params.importerId, validationResults);

            const affectedColumns = uniq(validationResults.map((result) => result.column));
        ...
        processDataValidationForRecord: async (params: {
            ...
            const validationResults = dataAnalyzer.processDataValidations([row], params.columnValidators, params.stats);
        ...
        applyPatches: async (params: {
            ...
            Context.current().log.info(`applying ${params.patches.length} patches`, params);
            const idempotencyKey = `patch-${Context.current().info.activityId}`;
            return await database.applyPatches(params.importerId, idempotencyKey, params.patches);
        },
        generateStatsPerColumn: async (params: {
            ...
            const columnStats = await database.getStatsPerColumn(params.importerId, params.uniqueColumns);
        ...
        invokeCallback: async (params: {
            ...
            if (!response.ok) {
                throw ApplicationFailure.retryable(`Failed to invoke callback ${response.status}`);
            }
        },
